...
4. **Detect hardware**: Run a quick GPU probe to determine GPU model, VRAM, and compute capability. Record the results — they set hard constraints on what experiments are feasible:
   ```python
   python -c "import torch; print(torch.cuda.get_device_name()); print(f'VRAM: {torch.cuda.get_device_properties(0).total_mem / 1024**3:.1f} GB'); print(f'Compute: sm_{torch.cuda.get_device_capability()[0]}{torch.cuda.get_device_capability()[1]}')"
   ```
...
5. Read out the results: `grep "^val_bpb:\|^peak_vram_mb:\|^num_steps:\|^mfu_percent:" run.log` (always check step count and MFU — they explain *why* a change helped or hurt)
